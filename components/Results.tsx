...
const Results: React.FC<ResultsProps> = ({ answers, userData, onNavigate, analysis, onSaveAnalysis }) => {
  ...
  // Atualiza estado local quando a prop muda (carregamento inicial)
  useEffect(() => {
  ...
  const generateAnalysis = async () => {
    ...
    try {
      ...
      if (newText) {
        ...
        onSaveAnalysis(newText); // Salva automaticamente no banco
      } else {
  ...
  const saveAnalysis = () => {
    ...
    onSaveAnalysis(editModal.text); // Salva a edição no banco
